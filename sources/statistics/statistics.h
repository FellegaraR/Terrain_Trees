...
private:
    ...
    /*!
     * \param mesh a Mesh& argument, represents the triangle mesh
     */
    inline void init_vector(Mesh& mesh) { this->indexStats.num_leaves_for_tri.assign(mesh.get_triangles_num(),0); }
    ...
    /**
     * @brief A private procedure that the statistics connected to the vertices indexed by a leaf
     * @param num_t_completely an integer containing the number of triangles completely indexed by the leaf
     * @param num_t_partially an integer containing the number of triangles partially indexed by the leaf
     * @param num_t_overlapping an integer containing the number of triangles simply crossing (i.e., no indexed vertices) by the leaf
     */
    void set_leaf_triangles_stats(int num_t_completely, int num_t_partially, int num_t_overlapping);
};

template<class T> void Statistics::get_index_statistics(T& tree, bool reindex)
{
    init_vector(tree.get_mesh());
    visit_tree(tree.get_root(),tree.get_mesh().get_domain(),0,tree.get_mesh(),tree.get_subdivision(),reindex);
