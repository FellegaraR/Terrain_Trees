...
public:
    ...
    static bool read_noised_field_value(Mesh& mesh, string mesh_name, dvect &original_vertex_fields);

    /**
     * @brief A public method that reads a file containing a soup of triangles
     *
     ...
     */
    static bool read_soup(Soup& soup, string path);
    /**
     * @brief A public method that read a soup of triangles
     *
     * @param mesh represents the soup to initialize
     * @param path a string argument, representing the path to the mesh file
     * @return bool value, true if the file is correctly readed, false otherwise
     *
     * NOTA: the input file can be in TRI or OFF format
     *
     */
    static bool read_vertices(Mesh& mesh, string path);
...
private:
     ...
     * \return a boolean value, true if the file is correctly readed, false otherwise
     */
    static bool read_mesh_off(Mesh& mesh, string path);
    ///A public method that reads a file containing a triangle mesh (TRI format)
    /*!
     * \param mesh a Mesh& argument, representing the mesh to initialize
     * \param path a string argument, representing the path to the mesh file
     * \return a boolean value, true if the file is correctly readed, false otherwise
     */
    static bool read_mesh_tri(Mesh& mesh, string path);
    /**
     ...
     */
    static void read_vertices_list(Mesh& mesh, ifstream &input, itype num_vertices);
...
template<class T, class N> bool Reader::read_tree(T& tree, N& root, string fileName)
{
    ...
    while (input.good())
    {
        ...
        Reader::read_node(current,input,tree.get_subdivision().son_number(), tree.get_mesh());

        if (!current->is_leaf())
        {
            for (int i = 0; i < tree.get_subdivision().son_number(); i++)
            {
                N* s = new N();
                current->set_son(s, i);
